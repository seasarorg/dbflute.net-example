...
namespace DfExample.DBFlute.CBean.CQ.BS {
    ...
    [System.Serializable]
    public abstract class AbstractBsVdSynonymMemberCQ : AbstractConditionQuery {

        public AbstractBsVdSynonymMemberCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel)
            : base(childQuery, sqlClause, aliasName, nestLevel) {}

        public override String getTableDbName() { return "VD_SYNONYM_MEMBER"; }
        public override String getTableSqlName() { return "VD_SYNONYM_MEMBER"; }
        ...
        //                                                                    Scalar Condition
        //                                                                    ================
        public SSQFunction<VdSynonymMemberCB> Scalar_Equal() {
            return xcreateSSQFunction("=");
        }

        public SSQFunction<VdSynonymMemberCB> Scalar_NotEqual() {
            return xcreateSSQFunction("<>");
        }

        public SSQFunction<VdSynonymMemberCB> Scalar_GreaterEqual() {
            return xcreateSSQFunction(">=");
        }

        public SSQFunction<VdSynonymMemberCB> Scalar_GreaterThan() {
            return xcreateSSQFunction(">");
        }

        public SSQFunction<VdSynonymMemberCB> Scalar_LessEqual() {
            return xcreateSSQFunction("<=");
        }

        public SSQFunction<VdSynonymMemberCB> Scalar_LessThan() {
            return xcreateSSQFunction("<");
        }

        protected SSQFunction<VdSynonymMemberCB> xcreateSSQFunction(String operand) {
            return new SSQFunction<VdSynonymMemberCB>(delegate(String function, SubQuery<VdSynonymMemberCB> subQuery) {
                xscalarSubQuery(function, subQuery, operand);
            });
        }

        protected void xscalarSubQuery(String function, SubQuery<VdSynonymMemberCB> subQuery, String operand) {
            assertObjectNotNull("subQuery<VdSynonymMemberCB>", subQuery);
            VdSynonymMemberCB cb = new VdSynonymMemberCB(); cb.xsetupForScalarCondition(this); subQuery.Invoke(cb);
            String subQueryPropertyName = keepScalarSubQuery(cb.Query()); // for saving query-value.
            registerScalarSubQuery(function, cb.Query(), subQueryPropertyName, operand);
        }
        public abstract String keepScalarSubQuery(VdSynonymMemberCQ subQuery);

        // ===============================================================================
        //                                                                  MySelf InScope
        //                                                                  ==============
        public void MyselfInScope(SubQuery<VdSynonymMemberCB> subQuery) {
            assertObjectNotNull("subQuery<VdSynonymMemberCB>", subQuery);
            VdSynonymMemberCB cb = new VdSynonymMemberCB(); cb.xsetupForInScopeRelation(this); subQuery.Invoke(cb);
            String subQueryPropertyName = keepMyselfInScopeSubQuery(cb.Query()); // for saving query-value.
            registerInScopeSubQuery(cb.Query(), "MEMBER_ID", "MEMBER_ID", subQueryPropertyName);
        }
        public abstract String keepMyselfInScopeSubQuery(VdSynonymMemberCQ subQuery);
