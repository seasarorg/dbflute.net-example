...
namespace DfExample.DBFlute.HowTo.Jp {
    ...
    public class BehaviorMiddleTest : ContainerTestCase {
        ...
        [MbUnit.Framework.Test, Quill(Tx.Rollback)]
        public void OutsideSql_SelectList_selectOptionMember_DateFromTo_Tx() {
            // ## Arrange ##
            String firstDate = "2003-02-25";
            String lastDate = "2006-09-04";
            String lastNextDate = "2006-09-05";
            String path = MemberBhv.PATH_selectOptionMember;
            OptionMemberPmb pmb = new OptionMemberPmb();
            pmb.SetFromFormalizedDate_FromDate(new DateTime(2003, 2, 25));
            pmb.SetToFormalizedDate_ToDate(new DateTime(2006, 9, 4));
            ...
            // ## Assert ##
            assertNotSame(0, memberList.Count);
            bool existsLastDate = false;
            foreach (OptionMember member in memberList) {
                String memberName = member.MemberName;
                DateTime? formalizedDatetime = member.FormalizedDatetime;
                log(memberName + ", " + formalizedDatetime);
                if (formalizedDatetime.Value.ToString("yyyy-MM-dd").Equals(lastDate)) {
                    existsLastDate = true;
                }
            }
            assertEquals(firstDate, pmb.FromFormalizedDate.Value.ToString("yyyy-MM-dd"));
            assertEquals(lastNextDate, pmb.ToFormalizedDate.Value.ToString("yyyy-MM-dd"));
        ...
        public void OutsideSql_AutoPaging_SelectPage() {
            ...
            assertNotSame(page1[0].UnpaidManId, page2[0].UnpaidManId);
            assertNotSame(page2[0].UnpaidManId, page3[0].UnpaidManId);
        ...
        public void OutsideSql_SelectEntity_selectUnpaidSummaryMember() {
            ...
            assertEquals(3, member.UnpaidManId);
