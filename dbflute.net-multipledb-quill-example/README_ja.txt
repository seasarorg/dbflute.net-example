...
MySQL本体は「dbflute.net-multipledb-quill-example/localdb/mysql」に配置されることを前提とする。
...
まず、create-mysql-db.batを実行する。
そして、grant-mysql-db.batを実行する。

- - - - - - - - - - - - - - - - - - - - - -
moon51さんからContributeして頂きました。
ありがとうございます！
- - - - - - - - - - - - - - - - - - - - - -


4. ReplaceSchemaタスクを実行する。

dbflute_exampledb/replace-schema.batを実行することで、
ExampleDBのテーブル構造がデータベースに作成され、テストデータも登録される。
