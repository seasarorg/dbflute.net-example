...
namespace DfExample.DBFlute.MemberDb.AllCommon.S2Dao {
    ...
    public class MbS2DaoInterceptor : AbstractInterceptor {
        ...
        //                                                                     Constructor
        //                                                                     ===========
        public MbS2DaoInterceptor(IDaoMetaDataFactory daoMetaDataFactory) {
        ...
        protected virtual object DispatchInvoking(IMethodInvocation invocation) {
            ...
            // - - - - - - - - - - - - -
            MbConditionBean cb = PreprocessConditionBean(invocation);
            ...
            MbSqlResultHandler sqlResultHandler = GetSqlResultHander();
            ...
            try {
                ...
                if (e.GetType().Equals(typeof(NotSingleRowUpdatedRuntimeException))) {
                    throw new MbEntityAlreadyUpdatedException((NotSingleRowUpdatedRuntimeException)e);
                }
                throw;
            } finally {
            ...
            if (logEnabled || existsSqlResultHandler) {
                ...
                if (existsSqlResultHandler) {
                    MbSqlResultInfo info = new MbSqlResultInfo();
                    info.Result = ret;
                    info.CommandName = method.Name;
                    info.DisplaySql = (String)MbInternalMapContext.GetObject("df:DisplaySql");
        ...
        protected void InitializeSqlCommand(IMethodInvocation invocation) {
            ...
            if (typeof(MbOutsideSqlDao).IsAssignableFrom(targetType)) {
                return; // Do nothing!
            } else {
        ...
        protected String GenerateSpecifiedOutsideSqlUniqueKey(IMethodInvocation invocation) {
            ...
            Object pmb = args[1];
            MbOutsideSqlOption option = (MbOutsideSqlOption)args[2];
            Type resultType = null;
            if (args.Length > 3) {
                resultType = args[3] is Type ? (Type)args[3] : args[3].GetType();
            }
            return MbOutsideSqlContext.GenerateSpecifiedOutsideSqlUniqueKey(invocation.Method.Name, path, pmb, option, resultType);
        }
        ...
        protected void LogInvocation(IMethodInvocation invocation) {
            ...
            if (IsSpecifiedOutsideSql(invocation)) {
                Object[] args = invocation.Arguments;
                MbOutsideSqlContext outsideSqlContext = GetOutsideSqlContext();
        ...
        protected static class InvokeNameStringUtil {
            public static String removeBasePrefixFromSimpleClassName(String simpleClassName) {
                if (!simpleClassName.StartsWith("MbBs")) {
                    return simpleClassName;
                }
                int prefixLength = "MbBs".Length;
                ...
                return "Mb" + simpleClassName.Substring(prefixLength);
            }
        }
        ...
        protected void LogSqlCommand(IMethodInvocation invocation, ISqlCommand cmd, DateTime beforeCmd, DateTime afterCmd) {
            Log("SqlCommand Initialization Cost: [" + MbTraceViewUtil.ConvertToPerformanceView(beforeCmd, afterCmd) + "]");
        }
        ...
        protected void LogReturn(IMethodInvocation invocation, Type retType, Object ret, DateTime before, DateTime after) {
            MethodBase method = invocation.Method;
            try {
                String daoResultPrefix = "===========/ [" + MbTraceViewUtil.ConvertToPerformanceView(before, after) + " ";
                ...
                } else if (typeof(MbEntity).IsAssignableFrom(retType)) {
                    if (ret == null) {
                        Log(daoResultPrefix + "(null)" + "]");
                    } else {
                        MbEntity entity = (MbEntity)ret;
    	...
    	protected bool IsSpecifiedOutsideSql(IMethodInvocation invocation) {
    	    return typeof(MbOutsideSqlDao).IsAssignableFrom(GetComponentDef(invocation).ComponentType);
    	}
        ...
        protected void SetupOutsideSqlContextSelect(IMethodInvocation invocation) {
            ...
            MbOutsideSqlOption option = GetOutsideSqlOption(args);
            ...
            MbOutsideSqlContext outsideSqlContext = new MbOutsideSqlContext();
    	...
    	protected MbOutsideSqlOption GetOutsideSqlOption(Object[] args) {
            return (MbOutsideSqlOption)args[2];
    	}
	
        protected void SetupOutsideSqlFetchNarrowingBean(Object pmb, MbOutsideSqlOption option) {
            if (pmb == null || !MbFetchNarrowingBeanContext.IsTheTypeFetchNarrowingBean(pmb.GetType())) {
                return;
            }
            MbFetchNarrowingBean fetchNarrowingBean = (MbFetchNarrowingBean)pmb;
        ...
        //                                     ConditionBean
        //                                     -------------
        protected MbConditionBean PreprocessConditionBean(IMethodInvocation invocation) {
            MbOutsideSqlContext outsideSqlContext = GetOutsideSqlContext();
            if (outsideSqlContext != null) {
                return null; // Because it has already finished setting up fetchNarrowingBean for outsideSql here.
            }
			
            MbConditionBean cb = null;
            {
                Object[] args = invocation.Arguments;
                ...
                if (!MbConditionBeanContext.IsTheArgumentConditionBean(arg0)) {// The argument is not condition-bean...
                    if (MbFetchNarrowingBeanContext.IsTheArgumentFetchNarrowingBean(arg0) && !IsSelectCountIgnoreFetchScopeMethod(invocation)) {
                        // Fetch-narrowing-bean and Not select count!
                        MbFetchNarrowingBeanContext.SetFetchNarrowingBeanOnThread((MbFetchNarrowingBean)arg0);
                    }
                    return null;
                }

                cb = (MbConditionBean)arg0;
            }

            if (IsSelectCountIgnoreFetchScopeMethod(invocation)) {
                cb.xsetupSelectCountIgnoreFetchScope();
            } else {
                MbFetchNarrowingBeanContext.SetFetchNarrowingBeanOnThread(cb);
            }
        ...
        public void PostprocessConditionBean(IMethodInvocation invocation, MbConditionBean cb) {
            if (cb == null) {
                return;
            }
            if (IsSelectCountIgnoreFetchScopeMethod(invocation)) {
        ...
        protected void InitializeContext() {
            if (MbConditionBeanContext.IsExistConditionBeanOnThread()
                || MbOutsideSqlContext.IsExistOutsideSqlContextOnThread()
                || MbFetchNarrowingBeanContext.IsExistFetchNarrowingBeanOnThread()
                || MbInternalMapContext.IsExistInternalMapOnThread()) { // means recursive invoking
        ...
        protected void CloseContext() {
            if (MbFetchNarrowingBeanContext.IsExistFetchNarrowingBeanOnThread()) {
                ...
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                MbFetchNarrowingBeanContext.GetFetchNarrowingBeanOnThread().RestoreIgnoredFetchNarrowing();
            }
            ClearAllCurrentContext();
        ...
        protected void SaveAllContextOnThread() {
            MbContextStack.SaveAllContextOnThread();
        }

        protected void RestoreAllContextOnThreadIfExists() {
            MbContextStack.RestoreAllContextOnThreadIfExists();
        }

        protected void ClearAllCurrentContext() {
            MbContextStack.ClearAllCurrentContext();
        }
        ...
        protected MbOutsideSqlContext GetOutsideSqlContext() {
            if (!MbOutsideSqlContext.IsExistOutsideSqlContextOnThread()) {
                return null;
            }
            return MbOutsideSqlContext.GetOutsideSqlContextOnThread();
        }
        ...
        protected virtual MbSqlResultHandler GetSqlResultHander() {
            if (!MbCallbackContext.IsExistCallbackContextOnThread()) {
                return null;
            }
            return MbCallbackContext.GetCallbackContextOnThread().SqlResultHandler;
        }
