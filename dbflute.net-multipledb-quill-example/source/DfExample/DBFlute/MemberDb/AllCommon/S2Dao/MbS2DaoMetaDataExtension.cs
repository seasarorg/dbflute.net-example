...
using DfExample.DBFlute.MemberDb.AllCommon.Annotation;
...
using DfExample.DBFlute.MemberDb.AllCommon.S2Dao.Internal.SqlCommand;
using DfExample.DBFlute.MemberDb.AllCommon.S2Dao.Internal.RsHandler;

namespace DfExample.DBFlute.MemberDb.AllCommon.S2Dao {

    public class MbS2DaoMetaDataExtension : DaoMetaDataImpl {
        ...
        //                                                                     Constructor
        //                                                                     ===========
        public MbS2DaoMetaDataExtension()
            : base() {}
        ...
        protected ISqlCommand InitializeSpecifiedOutsideSqlCommand(String sqlCommandKey, MbOutsideSqlContext outsideSqlContext) {
        ...
        protected override void SetupSelectMethodByAuto(MethodInfo methodInfo) {
            ...
            if (argTypes.Length == 1 && ValueTypes.GetValueType(argTypes[0]) == ValueTypes.OBJECT) {
                ...
                MbS2DaoSelectDynamicCommand dynamicCommand = CreateCustomizeSelectDynamicCommand(handler);
                cmd = dynamicCommand;
            } else {
        ...
        protected void AssertAutoQueryByDtoUnsupported(MethodInfo methodInfo, Type[] argTypes) {
            ...
            if (!MbConditionBeanContext.IsTheTypeConditionBean(firstArgType)) {
        ...
        protected override AbstractSqlCommand CreateInsertAutoDynamicCommand(MethodInfo methodInfo, IDataSource dataSource, ICommandFactory commandFactory, IBeanMetaData beanMetaData, string[] propertyNames) {
            return new MbInternalInsertAutoDynamicCommand(dataSource, commandFactory, beanMetaData, propertyNames);
        }
        
        protected override void SetupUpdateMethodByAuto(MethodInfo mi) {
            if (IsFirstArgumentConditionBean(mi)) {
                ISqlCommand cmd = new MbInternalUpdateQueryAutoDynamicCommand(_dataSource, _commandFactory);
        ...
        protected override AbstractSqlCommand CreateUpdateAutoDynamicCommand(MethodInfo method, IDataSource dataSource, ICommandFactory commandFactory, IBeanMetaData beanMetaData, string[] propertyNames) {
            MbInternalUpdateAutoDynamicCommand cmd = new MbInternalUpdateAutoDynamicCommand(dataSource, commandFactory, CreateBeanMetaData4UpdateDeleteByAuto(method), propertyNames);
        ...
        protected override AbstractSqlCommand CreateUpdateModifiedOnlyCommand(MethodInfo method, IDataSource dataSource, ICommandFactory commandFactory, IBeanMetaData beanMetaData, string[] propertyNames) {
            MbInternalUpdateModifiedOnlyCommand cmd = new MbInternalUpdateModifiedOnlyCommand(dataSource, commandFactory, CreateBeanMetaData4UpdateDeleteByAuto(method), propertyNames);
            cmd.VersionNoAutoIncrementOnMemory = IsUpdateVersionNoAutoIncrementOnMemory(method);
            cmd.IsCheckSingleRowUpdate = !IsNonstrictMethod(method);
            return cmd;
        }
        ...
        protected override void SetupDeleteMethodByAuto(MethodInfo mi) {
            if (IsFirstArgumentConditionBean(mi)) {
                ISqlCommand cmd = new MbInternalDeleteQueryAutoDynamicCommand(_dataSource, _commandFactory);
                _sqlCommands.Add(mi.Name, cmd);
                return;
            }
        ...
        protected override AbstractSqlCommand CreateDeleteAutoStaticCommand(MethodInfo method, IDataSource dataSource, ICommandFactory commandFactory, IBeanMetaData beanMetaData, string[] propertyNames) {
            MbInternalDeleteAutoStaticCommand cmd = new MbInternalDeleteAutoStaticCommand(dataSource, commandFactory, CreateBeanMetaData4UpdateDeleteByAuto(method), propertyNames);
            cmd.IsCheckSingleRowUpdate = !IsNonstrictMethod(method);
            return cmd;
        }
        ...
        protected bool IsFirstArgumentConditionBean(MethodInfo mi) {
            ...
            return pmbTypes.Length > 0 && typeof(MbConditionBean).IsAssignableFrom(pmbTypes[0]);
        }
        ...
        protected override void SetupSelectMethodByManual(MethodInfo mi, string sql) {
            ...
            if (parameterTypes != null && parameterTypes.Length > 0
                    && typeof(MbCursorHandler).IsAssignableFrom(parameterTypes[parameterTypes.Length - 1])) {
        ...
        //              Select
        //               - - -
        protected void SetupSpecifiedOutsideSqlSelectCommand(String sqlCommandKey, MethodInfo method, MbOutsideSqlContext outsideSqlContext) {
            // - - - - - - - - - - - - - - - - - - - - - - -
            ...
            } else if (typeof(MbCursorHandler).IsAssignableFrom(lastestArguementType)) {
                // - - - - - - - -
                // CursorHandling
                // - - - - - - - -
        ...
        protected IDataReaderHandler CreateSpecifiedOutsideSqlCustomizeBeanListResultSetHandler(IBeanMetaData specifiedBeanMetaData, Type customizeEntityType) {
            ...
            return new MbInternalBeanGenericListMetaDataResultSetHandler(specifiedBeanMetaData, rowCreator, relationRowCreator);
        }
        ...
        //             Execute
        //             - - - -
        protected void SetupSpecifiedOutsideSqlExecuteCommand(String sqlCommandKey, MethodInfo method, MbOutsideSqlContext outsideSqlContext) {
            // - - - - - - - - - - - - - - - - - - - - - - -
            // The attribute of Specified-OutsideSqlContext.
            // - - - - - - - - - - - - - - - - - - - - - - -
            String sql = outsideSqlContext.ReadFilteredOutsideSql(this.SqlFileEncoding, _dbms.Suffix);
        ...
        protected class InternalSpecifiedOusideSqlUpdateDynamicCommand : MbInternalUpdateDynamicCommand {
        ...
        //          Call Procedure
        //           - - - - - - -
        protected void SetupSpecifiedOutsideSqlCallCommand(String sqlCommandKey, MethodInfo method, MbOutsideSqlContext outsideSqlContext) {
            // - - - - - - - - - - - - - - - - - - - - - - -
            // The attribute of Specified-OutsideSqlContext.
            // - - - - - - - - - - - - - - - - - - - - - - -
            Object pmb = outsideSqlContext.ParameterBean;
            ...
            MbInternalProcedureCommand cmd = CreateInternalProcedureCommand(method, metaData);
            _sqlCommands.Add(sqlCommandKey, cmd);
        }

        protected MbInternalProcedureCommand CreateInternalProcedureCommand(MethodInfo method, InternalProcedureMetaData metaData) {
            return new MbInternalProcedureCommand(_dataSource, CreateDataReaderHandler(method), _commandFactory, _dataReaderFactory, metaData);
        }
        ...
        protected void RegisterSqlCommand(String sqlCommandKey, MethodInfo method, String sql, String[] argNames, Type[] argTypes, MbInternalUpdateDynamicCommand cmd) {
            cmd.Sql = sql;
            cmd.ArgNames = argNames;
            cmd.ArgTypes = argTypes;
            _sqlCommands.Add(sqlCommandKey, cmd);
        ...
        protected virtual MbS2DaoSelectDynamicCommand CreateCustomizeSelectDynamicCommand(IDataReaderHandler handler) {
            MbFetchNarrowingResultSetFactory customizeResultSetFactory = new MbFetchNarrowingResultSetFactory();
            return new MbS2DaoSelectDynamicCommand(_dataSource, _commandFactory, handler, customizeResultSetFactory);
        }
        ...
        //                                                                   Assist Helper
        //                                                                   =============
        protected MbDBMeta FindDBMeta() {
            Type beanType = this.BeanType;
            if (beanType == null) {
                return null;
            }
            if (!typeof(MbEntity).IsAssignableFrom(beanType)) {
                return null;
            }
            MbEntity entity = (MbEntity)Activator.CreateInstance(beanType);
    ...
    public class BeanMetaDataDBFluteDBMetaExtension : BeanMetaDataImpl {
        protected DfExample.DBFlute.MemberDb.AllCommon.JavaLike.List<IPropertyType> _primaryKeyList = new DfExample.DBFlute.MemberDb.AllCommon.JavaLike.ArrayList<IPropertyType>();
        ...
        protected override void SetupProperty(Type beanType, IDatabaseMetaData dbMetaData, IDbms dbms) {
            ...
            MbEntity entity = (MbEntity)ClassUtil.NewInstance(beanType);
        ...
        protected IPropertyType CreatePropertyTypeExtension(PropertyInfo pi, MbDBMeta dbmeta) {
        ...
        protected bool IsRelationProperty(PropertyInfo pi, MbDBMeta dbmeta) {
        ...
        protected bool IsPersistentProperty(PropertyInfo pi, MbDBMeta dbmeta) {
            String propertyName = pi.Name;
        ...
        protected bool IsEntity(Type beanType) {
            return typeof(MbEntity).IsAssignableFrom(beanType);
        }
    ...
    public class BeanMetaDataCacheHandler {
        ...
        public bool IsDBFluteEntity(Type beanClass) {
            return typeof(MbEntity).IsAssignableFrom(beanClass);
        }
