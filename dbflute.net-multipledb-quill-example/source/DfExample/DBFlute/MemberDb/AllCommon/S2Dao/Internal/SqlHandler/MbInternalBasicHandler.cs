...
using DfExample.DBFlute.MemberDb.AllCommon.S2Dao.Internal.Util;

namespace DfExample.DBFlute.MemberDb.AllCommon.S2Dao.Internal.SqlHandler {

    public class MbInternalBasicHandler {
        ...
        //                                                                     Constructor
        //                                                                     ===========
        public MbInternalBasicHandler(IDataSource ds, String sql, ICommandFactory commandFactory) {
            DataSource = ds;
            Sql = sql;
        ...
        protected virtual void LogSql(Object[] args, Type[] argTypes) {
            // [SqlLogHandler]
            MbSqlLogHandler sqlLogHandler = GetSqlLogHander();
            ...
            // [SqlLogRegistry]
            MbInternalSqlLogRegistry sqlLogRegistry = MbInternalSqlLogRegistryLocator.Instance;
        ...
        protected virtual String GetCompleteSql(Object[] args) {
            return MbInternalBindVariableUtil.GetCompleteSql(_sql, args);
        }

        protected virtual MbSqlLogHandler GetSqlLogHander() {
            if (!MbCallbackContext.IsExistCallbackContextOnThread()) {
                return null;
            }
            return MbCallbackContext.GetCallbackContextOnThread().SqlLogHandler;
        }
        ...
        protected virtual MbInternalSqlLog CreateInternalSqlLog(String displaySql, Object[] args, Type[] argTypes) {
            MbInternalSqlLog sqlLog = new MbInternalSqlLog();
        ...
        protected virtual String GetBindVariableText(Object bindVariable) {
            return MbInternalBindVariableUtil.GetBindVariableText(bindVariable);
        }
    ...
    public class DbExceptionHandler {
        ...
        public void HandleDbException(Exception e, IDbCommand cmd, bool uniqueConstraintValid, String displaySql) {
            ...
            //     ThrowEntityAlreadyExistsException(e, cmd, resource);
            // }
            if (e is MbDangerousResultSizeException) {
        ...
        protected bool IsUniqueConstraintException(DbException e) {
            // [UnderReview]: How do I get unique constraint on ADO.NET?
            // return MbConditionBeanContext.IsUniqueConstraintException(ExtractSQLState(e), e.ErrorCode);
            return false;
        }
        ...
        protected void ThrowSQLFailureException(Exception e, IDbCommand cmd, String displaySql) {
            ...
            msg = msg + "* * * * * * * * * */";
            throw new MbSQLFailureException(msg, e);
        }
        ...
        protected String ExtractBehaviorInvokeName() {
            Object behaviorInvokeName = MbInternalMapContext.GetObject("df:BehaviorInvokeName");
            if (behaviorInvokeName == null) {
                return null;
            }
            Object clientInvokeName = MbInternalMapContext.GetObject("df:ClientInvokeName");
            Object byPassInvokeName = MbInternalMapContext.GetObject("df:ByPassInvokeName");
            StringBuilder sb = new StringBuilder();
        ...
        protected MbConditionBean GetConditionBean() {
            return MbConditionBeanContext.GetConditionBeanOnThread();
        }

        protected bool HasOutsideSqlContext() {
            return MbOutsideSqlContext.IsExistOutsideSqlContextOnThread();
        }
