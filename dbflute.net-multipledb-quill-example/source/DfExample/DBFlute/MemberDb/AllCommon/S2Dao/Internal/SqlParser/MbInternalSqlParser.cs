...
namespace DfExample.DBFlute.MemberDb.AllCommon.S2Dao.Internal.SqlParser {

    public class MbInternalSqlParser : ISqlParser {
        ...
        protected readonly MbInternalSqlTokenizer _tokenizer;
        protected readonly System.Collections.Stack _nodeStack = new System.Collections.Stack();

        // ===============================================================================
        //                                                                     Constructor
        //                                                                     ===========
        public MbInternalSqlParser(String sql, bool blockNullParameter) {
            sql = sql.Trim();
            if (sql.EndsWith(";")) {
            ...
            _tokenizer = new MbInternalSqlTokenizer(sql);
        }
        ...
        protected void ParseSql() {
            ...
            if ((node is InternalIfNode || node is ElseNode) && node.ChildSize == 0) {
                MbInternalSqlTokenizer st = new MbInternalSqlTokenizer(sql);
                st.SkipWhitespace();
        ...
        protected void ThrowIfCommentConditionNotFoundException() {
            ...
            msg = msg + "* * * * * * * * * */";
            throw new MbIfCommentConditionNotFoundException(msg);
        }
        ...
        protected void ThrowEndCommentNotFoundException() {
            ...
            msg = msg + "[Specified SQL]" + GetLineSeparator() + _specifiedSql + GetLineSeparator();
            msg = msg + "* * * * * * * * * */";
            throw new MbEndCommentNotFoundException(msg);
        }
    	...
    	public static String ConvertTwoWaySql2DisplaySql(String twoWaySql, String[] argNames, Type[] argTypes, Object[] args) {
    		...
    		{
                MbInternalSqlParser parser = new MbInternalSqlParser(twoWaySql, false);
    		...
    		return MbInternalBindVariableUtil.GetCompleteSql(preparedSql, context.BindVariables);
    	}
    ...
    public class InternalValueAndTypeSetuper {
        ...
        public void SetupValueAndType(InternalValueAndType valueAndType) {
            ...
            MbLikeSearchOption likeSearchOption = null;
            ...
            for (int pos = 1; pos < _names.Length; ++pos) {
                ...
                if (pos == 1 && typeof(MbMapParameterBean).IsAssignableFrom(type)) {
                    MbMapParameterBean pmb = (MbMapParameterBean)value;
        ...
        // for OutsideSql
        protected bool IsLastLoop4LikeSearch(int pos, MbLikeSearchOption likeSearchOption) {
        ...
        // for OutsideSql
        protected MbLikeSearchOption GetLikeSearchOption(Type type, String currentName, Object resourceBean) {
            PropertyInfo pi = type.GetProperty(currentName + "InternalLikeSearchOption");
            MbLikeSearchOption option = (MbLikeSearchOption)pi.GetValue(resourceBean, null);
			if (option == null) {
        ...
        // for OutsideSql
        protected void ThrowOutsideSqlLikeSearchOptionSplitUnsupportedException(MbLikeSearchOption option, Object resourceBean, String currentName) {
            String msg = "Look! Read the message below." + GetLineSeparator();
            msg = msg + "/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *" + GetLineSeparator();
        ...
        protected String InitCap(String name) {
            return MbSimpleStringUtil.InitCap(name);
        }
        ...
        protected virtual void ThrowBindOrEmbeddedCommentNotFoundPropertyException(String expression, Type targetType, String notFoundProperty, String specifiedSql, bool bind) {
            ...
            msg = msg + "* * * * * * * * * */";
            if (bind) {
                throw new MbBindVariableCommentNotFoundPropertyException(msg);
            } else {
                throw new MbEmbeddedValueCommentNotFoundPropertyException(msg);
            }
        }
	...
	public class ParameterCommentExceptionProvider {
        public static void ThrowBindOrEmbeddedParameterNullValueException(String expression, Type targetType, String specifiedSql, bool bind) {
            ...
            msg = msg + "* * * * * * * * * */";
            if (bind) {
                throw new MbBindVariableParameterNullValueException(msg);
            } else {
                throw new MbEmbeddedValueParameterNullValueException(msg);
            }
        }
    ...
    public class InternalIfNode : ContainerNode {
        ...
        protected virtual void ThrowIfCommentWrongExpressionException(String expression, Exception cause, String specifiedSql) {
            ...
            msg = msg + "[Specified SQL]" + GetLineSeparator() + specifiedSql + GetLineSeparator();
            msg = msg + "* * * * * * * * * */";
            throw new MbIfCommentWrongExpressionException(msg, cause);
        }
        ...
        protected virtual void ThrowIfCommentNotBooleanResultException(String expression, Object result, String specifiedSql) {
            ...
            msg = msg + "[Specified SQL]" + GetLineSeparator() + specifiedSql + GetLineSeparator();
            msg = msg + "* * * * * * * * * */";
            throw new MbIfCommentNotBooleanResultException(msg);
        }
