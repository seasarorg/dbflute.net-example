...
namespace DfExample.DBFlute.MemberDb.AllCommon {
    ...
    public class MbAccessContext {

        private static LocalDataStoreSlot _accessContextSlot = Thread.AllocateDataSlot();

        public static MbAccessContext GetAccessContextOnThread() {
            return (MbAccessContext)Thread.GetData(_accessContextSlot);
        }

        public static void SetAccessContextOnThread(MbAccessContext context) {
            if (context == null) {
		...
		public static DateTime? GetAccessTimestampOnThread() {
            if (IsExistAccessContextOnThread()) {
                MbAccessContext userContextOnThread = GetAccessContextOnThread();
