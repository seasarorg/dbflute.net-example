...
// JavaLike
[System.Serializable]
public abstract class LdAbstractSqlClause : LdSqlClause {
    ...
    protected Map<String, LdConditionKey> _invalidQueryColumnMap;
    ...
    public String getSelectClause() {
        ...
        // - - - - - - - - - -/
        StringBuilder sb = new StringBuilder();
        LdDBMeta dbmeta = getDBMeta();
        List<LdColumnInfo> columnInfoList = dbmeta.ColumnInfoList;
        ...
        foreach (LdColumnInfo columnInfo in columnInfoList) {
    ...
    protected String getLeftOuterJoinClause() {
        ...
        for (Iterator<String> ite = _outerJoinMap.keySet().iterator(); ite.hasNext(); ) {
            ...
            LdDBMeta foreignDBMeta = findDBMeta(foreignTableDbName);
    ...
    protected Map<String, SelectedSelectColumnInfo> createSelectedSelectColumnInfo(String foreignTableAliasName
                                                                                 ...
                                                                                 , String foreignPropertyName
                                                                                 , String localRelationPath) {
        LdDBMeta dbmeta = findDBMeta(localTableName);
        LdForeignInfo foreignInfo = dbmeta.FindForeignInfo(foreignPropertyName);
        ...
        LdDBMeta foreignDBMeta = foreignInfo.ForeignDBMeta;
        foreach (LdColumnInfo columnInfo in foreignDBMeta.ColumnInfoList) {
            String columnDbName = columnInfo.ColumnDbName;
            String columnSqlName = columnInfo.ColumnSqlName;
    ...
    public LdSqlClause makeInnerJoinEffective() {
        _innerJoinEffective = true;
        return this;
    }

    public LdSqlClause backToOuterJoin() {
    ...
    protected void doRegisterWhereClause(List<String> clauseList, String columnName, LdConditionKey key,
            LdConditionValue value) {
        key.addWhereClause(clauseList, columnName, value);
        markOrScopeQueryAndPart(clauseList);
    }

    protected void doRegisterWhereClause(List<String> clauseList, String columnName, LdConditionKey key,
            LdConditionValue value, LdConditionOption option) {
    ...
    public void registerOuterJoinInlineWhereClause(String aliasName, String columnName, LdConditionKey key, LdConditionValue value, bool onClause) {
    ...
    public void registerOuterJoinInlineWhereClause(String aliasName, String columnName, LdConditionKey key, LdConditionValue value, LdConditionOption option, bool onClause) {
        assertNotYetOuterJoin(aliasName);
        assertStringNotNullAndNotTrimmedEmpty("columnName", columnName);
        List<String> clauseList = getOuterJoinInlineWhereClauseList4Register(aliasName, onClause);
        String realColumnName = (onClause ? aliasName + "." : "") + columnName;
    ...
    public virtual LdSqlClause ignoreFetchScope() {
        _fetchScopeEffective = false;
        doClearFetchPageClause();
        return this;
    }

    public virtual LdSqlClause makeFetchScopeEffective() {
    ...
    //                                                                                Lock
    //                                                                                ====
    public abstract LdSqlClause lockForUpdate();
...
//        }
//        for (Iterator<LdWhereClauseSimpleFilter> ite = _whereClauseSimpleFilterList.iterator(); ite.hasNext(); ) {
//             LdWhereClauseSimpleFilter filter = ite.next();
//            if (filter == null) {
...
//    }
    ...
    public Map<String, LdConditionKey> getInvalidQueryColumnMap() {
        if (_invalidQueryColumnMap != null) {
            return _invalidQueryColumnMap;
        }
        return new HashMap<String, LdConditionKey>();
    }

    public void registerInvalidQueryColumn(String columnFullName, LdConditionKey key) {
        if (_invalidQueryColumnMap == null) {
            _invalidQueryColumnMap = new LinkedHashMap<String, LdConditionKey>();
        }
    ...
    protected LdDBMeta findDBMeta(String tableDbName) {
        return LdDBMetaInstanceHandler.FindDBMeta(tableDbName); 
    }
    ...
    protected String toStringView(Object obj) {
        return LdTraceViewUtil.ToStringView(obj);
    }
