...
namespace DfExample.DBFlute.LibraryDb.AllCommon.S2Dao {
    ...
    public class LdS2DaoInterceptor : AbstractInterceptor {
        ...
        //                                                                     Constructor
        //                                                                     ===========
        public LdS2DaoInterceptor(IDaoMetaDataFactory daoMetaDataFactory) {
        ...
        protected virtual object DispatchInvoking(IMethodInvocation invocation) {
            ...
            // - - - - - - - - - - - - -
            LdConditionBean cb = PreprocessConditionBean(invocation);
            ...
            LdSqlResultHandler sqlResultHandler = GetSqlResultHander();
            ...
            try {
                ...
                if (e.GetType().Equals(typeof(NotSingleRowUpdatedRuntimeException))) {
                    throw new LdEntityAlreadyUpdatedException((NotSingleRowUpdatedRuntimeException)e);
                }
                throw;
            } finally {
            ...
            if (logEnabled || existsSqlResultHandler) {
                ...
                if (existsSqlResultHandler) {
                    LdSqlResultInfo info = new LdSqlResultInfo();
                    info.Result = ret;
                    info.CommandName = method.Name;
                    info.DisplaySql = (String)LdInternalMapContext.GetObject("df:DisplaySql");
        ...
        protected void InitializeSqlCommand(IMethodInvocation invocation) {
            ...
            if (typeof(LdOutsideSqlDao).IsAssignableFrom(targetType)) {
                return; // Do nothing!
            } else {
        ...
        protected String GenerateSpecifiedOutsideSqlUniqueKey(IMethodInvocation invocation) {
            ...
            Object pmb = args[1];
            LdOutsideSqlOption option = (LdOutsideSqlOption)args[2];
            Type resultType = null;
            if (args.Length > 3) {
                resultType = args[3] is Type ? (Type)args[3] : args[3].GetType();
            }
            return LdOutsideSqlContext.GenerateSpecifiedOutsideSqlUniqueKey(invocation.Method.Name, path, pmb, option, resultType);
        }
        ...
        protected void LogInvocation(IMethodInvocation invocation) {
            ...
            if (IsSpecifiedOutsideSql(invocation)) {
                Object[] args = invocation.Arguments;
                LdOutsideSqlContext outsideSqlContext = GetOutsideSqlContext();
        ...
        protected void LogSqlCommand(IMethodInvocation invocation, ISqlCommand cmd, DateTime beforeCmd, DateTime afterCmd) {
            Log("SqlCommand Initialization Cost: [" + LdTraceViewUtil.ConvertToPerformanceView(beforeCmd, afterCmd) + "]");
        }
        ...
        protected void LogReturn(IMethodInvocation invocation, Type retType, Object ret, DateTime before, DateTime after) {
            MethodBase method = invocation.Method;
            try {
                String daoResultPrefix = "===========/ [" + LdTraceViewUtil.ConvertToPerformanceView(before, after) + " ";
                ...
                } else if (typeof(LdEntity).IsAssignableFrom(retType)) {
                    if (ret == null) {
                        Log(daoResultPrefix + "(null)" + "]");
                    } else {
                        LdEntity entity = (LdEntity)ret;
    	...
    	protected bool IsSpecifiedOutsideSql(IMethodInvocation invocation) {
    	    return typeof(LdOutsideSqlDao).IsAssignableFrom(GetComponentDef(invocation).ComponentType);
    	}
        ...
        protected void SetupOutsideSqlContextSelect(IMethodInvocation invocation) {
            ...
            LdOutsideSqlOption option = GetOutsideSqlOption(args);
            ...
            LdOutsideSqlContext outsideSqlContext = new LdOutsideSqlContext();
    	...
    	protected LdOutsideSqlOption GetOutsideSqlOption(Object[] args) {
            return (LdOutsideSqlOption)args[2];
    	}
	
        protected void SetupOutsideSqlFetchNarrowingBean(Object pmb, LdOutsideSqlOption option) {
            if (pmb == null || !LdFetchNarrowingBeanContext.IsTheTypeFetchNarrowingBean(pmb.GetType())) {
                return;
            }
            LdFetchNarrowingBean fetchNarrowingBean = (LdFetchNarrowingBean)pmb;
        ...
        //                                     ConditionBean
        //                                     -------------
        protected LdConditionBean PreprocessConditionBean(IMethodInvocation invocation) {
            LdOutsideSqlContext outsideSqlContext = GetOutsideSqlContext();
            if (outsideSqlContext != null) {
                return null; // Because it has already finished setting up fetchNarrowingBean for outsideSql here.
            }
			
            LdConditionBean cb = null;
            {
                Object[] args = invocation.Arguments;
                ...
                if (!LdConditionBeanContext.IsTheArgumentConditionBean(arg0)) {// The argument is not condition-bean...
                    if (LdFetchNarrowingBeanContext.IsTheArgumentFetchNarrowingBean(arg0) && !IsSelectCountIgnoreFetchScopeMethod(invocation)) {
        ...
        public void PostprocessConditionBean(IMethodInvocation invocation, LdConditionBean cb) {
            if (cb == null) {
                return;
            }
            if (IsSelectCountIgnoreFetchScopeMethod(invocation)) {
        ...
        protected void InitializeContext() {
            if (LdConditionBeanContext.IsExistConditionBeanOnThread()
                || LdOutsideSqlContext.IsExistOutsideSqlContextOnThread()
                || LdFetchNarrowingBeanContext.IsExistFetchNarrowingBeanOnThread()
                || LdInternalMapContext.IsExistInternalMapOnThread()) { // means recursive invoking
        ...
        protected void SaveAllContextOnThread() {
            LdContextStack.SaveAllContextOnThread();
        }

        protected void RestoreAllContextOnThreadIfExists() {
            LdContextStack.RestoreAllContextOnThreadIfExists();
        }

        protected void ClearAllCurrentContext() {
            LdContextStack.ClearAllCurrentContext();
        }
        ...
        protected virtual LdSqlResultHandler GetSqlResultHander() {
            if (!LdCallbackContext.IsExistCallbackContextOnThread()) {
                return null;
            }
            return LdCallbackContext.GetCallbackContextOnThread().SqlResultHandler;
        }
