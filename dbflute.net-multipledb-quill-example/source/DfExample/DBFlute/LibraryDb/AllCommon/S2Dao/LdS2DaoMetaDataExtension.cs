...
using DfExample.DBFlute.LibraryDb.AllCommon.Annotation;
...
using DfExample.DBFlute.LibraryDb.AllCommon.S2Dao.Internal.SqlCommand;
using DfExample.DBFlute.LibraryDb.AllCommon.S2Dao.Internal.RsHandler;

namespace DfExample.DBFlute.LibraryDb.AllCommon.S2Dao {

    public class LdS2DaoMetaDataExtension : DaoMetaDataImpl {
        ...
        protected ISqlCommand InitializeSpecifiedOutsideSqlCommand(String sqlCommandKey, LdOutsideSqlContext outsideSqlContext) {
        ...
        protected override void SetupSelectMethodByAuto(MethodInfo methodInfo) {
            ...
            if (argTypes.Length == 1 && ValueTypes.GetValueType(argTypes[0]) == ValueTypes.OBJECT) {
                ...
                LdS2DaoSelectDynamicCommand dynamicCommand = CreateCustomizeSelectDynamicCommand(handler);
                cmd = dynamicCommand;
            } else {
        ...
        protected void AssertAutoQueryByDtoUnsupported(MethodInfo methodInfo, Type[] argTypes) {
            ...
            if (!LdConditionBeanContext.IsTheTypeConditionBean(firstArgType)) {
        ...
        protected override AbstractSqlCommand CreateInsertAutoDynamicCommand(MethodInfo methodInfo, IDataSource dataSource, ICommandFactory commandFactory, IBeanMetaData beanMetaData, string[] propertyNames) {
            return new LdInternalInsertAutoDynamicCommand(dataSource, commandFactory, beanMetaData, propertyNames);
        }
        
        protected override void SetupUpdateMethodByAuto(MethodInfo mi) {
            if (IsFirstArgumentConditionBean(mi)) {
                ISqlCommand cmd = new LdInternalUpdateQueryAutoDynamicCommand(_dataSource, _commandFactory);
        ...
        protected override AbstractSqlCommand CreateUpdateAutoDynamicCommand(MethodInfo method, IDataSource dataSource, ICommandFactory commandFactory, IBeanMetaData beanMetaData, string[] propertyNames) {
            LdInternalUpdateAutoDynamicCommand cmd = new LdInternalUpdateAutoDynamicCommand(dataSource, commandFactory, CreateBeanMetaData4UpdateDeleteByAuto(method), propertyNames);
        ...
        protected override AbstractSqlCommand CreateUpdateModifiedOnlyCommand(MethodInfo method, IDataSource dataSource, ICommandFactory commandFactory, IBeanMetaData beanMetaData, string[] propertyNames) {
            LdInternalUpdateModifiedOnlyCommand cmd = new LdInternalUpdateModifiedOnlyCommand(dataSource, commandFactory, CreateBeanMetaData4UpdateDeleteByAuto(method), propertyNames);
            cmd.VersionNoAutoIncrementOnMemory = IsUpdateVersionNoAutoIncrementOnMemory(method);
            cmd.IsCheckSingleRowUpdate = !IsNonstrictMethod(method);
            return cmd;
        }
        ...
        protected override void SetupDeleteMethodByAuto(MethodInfo mi) {
            if (IsFirstArgumentConditionBean(mi)) {
                ISqlCommand cmd = new LdInternalDeleteQueryAutoDynamicCommand(_dataSource, _commandFactory);
                _sqlCommands.Add(mi.Name, cmd);
                return;
            }
        ...
        protected override AbstractSqlCommand CreateDeleteAutoStaticCommand(MethodInfo method, IDataSource dataSource, ICommandFactory commandFactory, IBeanMetaData beanMetaData, string[] propertyNames) {
            LdInternalDeleteAutoStaticCommand cmd = new LdInternalDeleteAutoStaticCommand(dataSource, commandFactory, CreateBeanMetaData4UpdateDeleteByAuto(method), propertyNames);
            cmd.IsCheckSingleRowUpdate = !IsNonstrictMethod(method);
            return cmd;
        }
        ...
        protected bool IsFirstArgumentConditionBean(MethodInfo mi) {
            ...
            return pmbTypes.Length > 0 && typeof(LdConditionBean).IsAssignableFrom(pmbTypes[0]);
        }
        ...
        protected override void SetupSelectMethodByManual(MethodInfo mi, string sql) {
            ...
            if (parameterTypes != null && parameterTypes.Length > 0
                    && typeof(LdCursorHandler).IsAssignableFrom(parameterTypes[parameterTypes.Length - 1])) {
        ...
        //              Select
        //               - - -
        protected void SetupSpecifiedOutsideSqlSelectCommand(String sqlCommandKey, MethodInfo method, LdOutsideSqlContext outsideSqlContext) {
            // - - - - - - - - - - - - - - - - - - - - - - -
            ...
            } else if (typeof(LdCursorHandler).IsAssignableFrom(lastestArguementType)) {
                // - - - - - - - -
                // CursorHandling
                // - - - - - - - -
        ...
        protected IDataReaderHandler CreateSpecifiedOutsideSqlCustomizeBeanListResultSetHandler(IBeanMetaData specifiedBeanMetaData, Type customizeEntityType) {
            ...
            return new LdInternalBeanGenericListMetaDataResultSetHandler(specifiedBeanMetaData, rowCreator, relationRowCreator);
        }
        ...
        //             Execute
        //             - - - -
        protected void SetupSpecifiedOutsideSqlExecuteCommand(String sqlCommandKey, MethodInfo method, LdOutsideSqlContext outsideSqlContext) {
            // - - - - - - - - - - - - - - - - - - - - - - -
            // The attribute of Specified-OutsideSqlContext.
            // - - - - - - - - - - - - - - - - - - - - - - -
            String sql = outsideSqlContext.ReadFilteredOutsideSql(this.SqlFileEncoding, _dbms.Suffix);
        ...
        protected class InternalSpecifiedOusideSqlUpdateDynamicCommand : LdInternalUpdateDynamicCommand {
        ...
        //          Call Procedure
        //           - - - - - - -
        protected void SetupSpecifiedOutsideSqlCallCommand(String sqlCommandKey, MethodInfo method, LdOutsideSqlContext outsideSqlContext) {
            // - - - - - - - - - - - - - - - - - - - - - - -
            // The attribute of Specified-OutsideSqlContext.
            // - - - - - - - - - - - - - - - - - - - - - - -
            Object pmb = outsideSqlContext.ParameterBean;
            ...
            LdInternalProcedureCommand cmd = CreateInternalProcedureCommand(method, metaData);
            _sqlCommands.Add(sqlCommandKey, cmd);
        }

        protected LdInternalProcedureCommand CreateInternalProcedureCommand(MethodInfo method, InternalProcedureMetaData metaData) {
            return new LdInternalProcedureCommand(_dataSource, CreateDataReaderHandler(method), _commandFactory, _dataReaderFactory, metaData);
        }
        ...
        protected void RegisterSqlCommand(String sqlCommandKey, MethodInfo method, String sql, String[] argNames, Type[] argTypes, LdInternalUpdateDynamicCommand cmd) {
            cmd.Sql = sql;
            cmd.ArgNames = argNames;
            cmd.ArgTypes = argTypes;
            _sqlCommands.Add(sqlCommandKey, cmd);
        ...
        protected virtual LdS2DaoSelectDynamicCommand CreateCustomizeSelectDynamicCommand(IDataReaderHandler handler) {
            LdFetchNarrowingResultSetFactory customizeResultSetFactory = new LdFetchNarrowingResultSetFactory();
            return new LdS2DaoSelectDynamicCommand(_dataSource, _commandFactory, handler, customizeResultSetFactory);
        }
        ...
        protected LdDBMeta FindDBMeta() {
            ...
            if (!typeof(LdEntity).IsAssignableFrom(beanType)) {
                return null;
            }
            LdEntity entity = (LdEntity)Activator.CreateInstance(beanType);
    ...
    public class BeanMetaDataDBFluteDBMetaExtension : BeanMetaDataImpl {
        protected DfExample.DBFlute.LibraryDb.AllCommon.JavaLike.List<IPropertyType> _primaryKeyList = new DfExample.DBFlute.LibraryDb.AllCommon.JavaLike.ArrayList<IPropertyType>();
        ...
        protected override void SetupProperty(Type beanType, IDatabaseMetaData dbMetaData, IDbms dbms) {
            ...
            LdEntity entity = (LdEntity)ClassUtil.NewInstance(beanType);
        ...
        protected IPropertyType CreatePropertyTypeExtension(PropertyInfo pi, LdDBMeta dbmeta) {
        ...
        protected bool IsRelationProperty(PropertyInfo pi, LdDBMeta dbmeta) {
        ...
        protected bool IsPersistentProperty(PropertyInfo pi, LdDBMeta dbmeta) {
            String propertyName = pi.Name;
        ...
        protected bool IsEntity(Type beanType) {
            return typeof(LdEntity).IsAssignableFrom(beanType);
        }
    ...
    public class BeanMetaDataCacheHandler {
        ...
        public bool IsDBFluteEntity(Type beanClass) {
            return typeof(LdEntity).IsAssignableFrom(beanClass);
        }
